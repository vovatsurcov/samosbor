...
test("неполное сохранение без зоны безопасно возвращается на стартовый этаж", () => {
  ...
  assert.ok(migrated.hero.positions.floor544);
  // Сохранение без поля stance — со старой шкалы ОЗ (максимум 8). Миграция v2
  // переводит его в новую шкалу по доле здоровья, а не копирует число буквально.
  assert.equal(migrated.hero.hp, Math.round((3 / 8) * maxHeroHp(migrated)));
  assert.ok(migrated.hero.hp > 3);
});
