...
export function objectiveFor(state: any): string {
  const ensured = normalizeCityState(state);
  // Сирена и активная фаза важнее любой плановой задачи: пока Самосбор идёт,
  // герою нельзя показывать «сходить к диспетчеру» вместо «уйти за гермодверь».
  const samosborPhase = base.samosborPhaseIn(ensured, ensured.zone);
  if (isSamosborWarningPhase(samosborPhase) || isSamosborLethalPhase(samosborPhase)) {
    return base.objectiveFor(ensured);
  }
