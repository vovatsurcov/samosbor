...
function awardQuestXp<T extends CampaignHost>(state: T, questId: StarterQuestId): T & { campaign: StarterCampaignState } {
  ...
  if (campaign.rewardedQuests.includes(questId) || definition.xpRatio <= 0) return ensureStarterCampaign(state);
...
export function recordDispatcherBriefing<T extends CampaignHost>(state: T): T & { campaign: StarterCampaignState } {
  ...
  next = { ...next, campaign: { ...next.campaign, dispatcherBriefed: true } };
...
export function recordCityServiceVisit<T extends CampaignHost>(
  ...
  next = {
    ...
    campaign: {
      ...next.campaign,
      cityServicesVisited: [...new Set([...next.campaign.cityServicesVisited, service])],
    },
  };
...
export function chooseStarterApproach<T extends CampaignHost>(
  ...
  return appendCampaignLog(
    { ...next, campaign: { ...next.campaign, approach } },
    approach === "scout"
...
export function tickStarterCampaign<T extends CampaignHost>(state: T): T & { campaign: StarterCampaignState } {
  ...
  if (
    next.campaign.currentQuest === "START-005" &&
    next.campaign.cityServicesVisited.length >= 3 &&
    Boolean(next.professions?.trialPermit)
  ) {
    next = completeQuest(next, "START-005");
  }
...
export function starterObjective(state: CampaignHost): string | null {
  ...
  if (campaign.currentQuest === "START-005") {
    const permit = state.professions?.trialPermit ? "допуск получен" : "нужен пробный допуск";
    return `${quest.id} · ${quest.title}: городские службы ${campaign.cityServicesVisited.length}/3 · ${permit}`;
  }
