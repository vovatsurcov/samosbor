...
export type TalentBonusKey =
  ...
  | "healing"
  | "stressResist"
  // Параметры защитного действия и заряда: пассивы меняют бой через данные,
  // не переписывая базовую боевую систему.
  | "blockHold"
  | "blockCostReduction"
  | "staggerResist"
  | "parryWindow"
...
const BRANCH_ENTRIES: Record<SkillBranch, BranchEntry[]> = {
  ...
  guard: [
    ["Уверенный упор", "+1 брони.", { armor: 1 }, ["defense"]],
    ["Ремень снаряжения", "+3 кг грузоподъёмности.", { carry: 3 }],
    ["Защитная стойка", "Позволяет удерживать блок, а не только ставить его на миг.", { blockHold: 1 }, ["protocol", "block"]],
    ["Ровное дыхание", "Удержание блока тратит на 20% меньше дыхания.", { blockCostReduction: 0.2 }, ["block"]],
    ["Протокол упора", "+4% поглощения блока.", { block: 0.04 }, ["protocol", "block"]],
    ["Широкий хват", "+5% поглощения блока.", { block: 0.05 }, ["block"]],
    ["Устойчивость", "Входящий урон по стойке снижен на 15%.", { staggerResist: 0.15 }, ["stagger"]],
    ["Плотный контур", "+2 брони.", { armor: 2 }, ["defense"]],
    ["Регламент удержания", "Провокация собирает противников плотнее.", { tauntRadius: 2 }, ["protocol", "control"]],
    ["Второе плечо", "+6% поглощения блока и +1 ОЗ на уровень контура.", { block: 0.06, maxHp: 1 }, ["notable", "block"]],
    ["Ровно на ногах", "Ещё 15% сопротивления урону по стойке.", { staggerResist: 0.15 }, ["stagger"]],
    ["Экономный блок", "Удержание блока тратит ещё на 25% меньше дыхания.", { blockCostReduction: 0.25 }, ["block"]],
    ["Протокол гермодвери", "+8% поглощения блока.", { block: 0.08 }, ["protocol", "block"]],
    ["Держать проход", "Провокация и броня усиливаются в узких местах.", { tauntRadius: 2, armor: 2 }, ["control"]],
    ["Несдвигаемый", "+3 брони и +12% сопротивления ошеломлению.", { armor: 3, staggerResist: 0.12 }, ["notable", "defense"]],
    ["Нарушение: контур не сдаётся", "Блок не ломается при нулевом дыхании, но каждый удар режет стойку.", { blockHold: 1, block: 0.1, staggerResist: 0.2, blockCostReduction: 0.2 }, ["keystone", "block"]],
  ],
  agility: [
    ["Лёгкий шаг", "+4% к скорости перемещения.", { moveSpeed: 0.04 }, ["movement"]],
    ["Тихая подошва", "Снижает шум шагов.", { noiseReduction: 0.2 }, ["stealth"]],
    ["Короткое парирование", "Защитное действие получает окно парирования.", { parryWindow: 120 }, ["protocol", "parry"]],
    ["Чтение замаха", "Окно парирования шире на 60 мс.", { parryWindow: 60 }, ["parry"]],
    ...
    ["Обратный удар", "Парирование возвращает часть урона атакующему.", { parryReflect: 0.3 }, ["parry"]],
    ...
    ["Длинное окно", "Окно парирования шире ещё на 60 мс.", { parryWindow: 60 }, ["parry"]],
    ...
    ["Зеркальный ответ", "Парирование отражает вдвое больше.", { parryReflect: 0.3 }, ["parry"]],
    ["Работа по слабому", "+4% критического шанса.", { critical: 0.04 }, ["notable"]],
    ["Нарушение: чужой темп", "Успешное парирование полностью снимает откат уклонения.", { parryWindow: 80, parryReflect: 0.4, attackSpeed: 0.06 }, ["keystone", "parry"]],
  ],
  precision: [
...
const HYBRID_DEFINITIONS: [SkillBranch, SkillBranch, string, string, TalentBonuses][] = [
  ...
  ["guard", "agility", "Встречный контур", "Парирование из защитной стойки дешевле по дыханию.", { parryWindow: 60, blockCostReduction: 0.2 }],
