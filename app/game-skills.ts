...
const BRANCH_ENTRIES: Record<SkillBranch, BranchEntry[]> = {
  power: [
    ...
    ["Регламент пролома", "ЗАМЕНЯЕТ ПКМ: сильная атака становится проламывающим ударом — вкладывает накопленный разгон в один удар и тратит его целиком.", { meleeDamage: 2 }, ["protocol", "transform", "strong"]],
  ...
  guard: [
    ...
    ["Нарушение: глухой контур", "Блок почти перестаёт гасить урон, зато каждый сработавший блок обращается в разгон.", { block: 0.18 }, ["keystone", "block", "rule"]],
  ],
  agility: [
    ...
    ["Чтение замаха", "ЗАМЕНЯЕТ SHIFT: ускорение уступает место короткому рывку с кадрами неуязвимости.", { evasion: 0.04 }, ["evasion", "transform", "movement"]],
    ...
    ["Нарушение: чужой темп", "Темп больше не затухает, но блок и парирование не работают совсем.", { attackSpeed: 0.06 }, ["keystone", "tempo", "rule"]],
  ],
  precision: [
  ...
  resonance: [
    ...
    ["Стабильный контур", "ЗАМЕНЯЕТ SHIFT: вместо ускорения — пространственный перенос в сторону курсора. Платится заражением, а не дыханием.", { voidStability: 0.06 }, ["anomaly", "transform", "movement"]],
    ...
    ["Регламент резонанса", "ЗАМЕНЯЕТ ЛКМ: обычная атака сама наводит резонанс, но бьёт слабее.", { markPower: 2 }, ["protocol", "transform", "primary"]],
